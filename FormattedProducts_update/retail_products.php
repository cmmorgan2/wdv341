<?php
    ...
    /*
        ...
        */
        ...
        include 'dbConnect.php';  //getting db file from in the same folder

        try {
        $sql = "SELECT product_name,product_description, product_price,product_status,product_image,product_inStock FROM wdv341_products;"; //selecting the information / product info from each product selection in folder edv341_products
        ...
        catch(PDOException $e){  // when catch finds  failure or error
            echo "Errors: " . $e->getMessage();  //puts out an error message
...
<body>
    ...
    <section>
        <?php
        foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $result)// for each selection go grab all the info for that product and display it
            {
        ?>
